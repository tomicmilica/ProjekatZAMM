...
{
    public class ProcessDES : CommonProcess
    {
        public ProcessDES()
        {
        ...
        public string DoPermutation(string text, int[] order)
        {
            ...
            Console.WriteLine(PermutatedText);
        ...
        #region KeyµéŔ» ±¸ÇĎ´Â ¸ŢĽ­µĺ - Get all of keys
        public Keys SetAllKeys(string C0, string D0)
        {
            ...
            Console.WriteLine("Kljuc {0}",keys.Kn);
            return keys;
        }
        #endregion
        ...
        public string FinalEncription(string L0, string R0, Keys keys, bool IsReverse)
        {
            ...
            int i = 0; //u enkripciji ide od L pocetka L0

            if (IsReverse == true)
            {
                i = 15;    //u dekripciji sa L15
            }

            while (this.IsEnough(i, IsReverse))
            {
                Ln = Rn_1;
                Rn = this.XOR(Ln_1, this.f(Rn_1, keys.Kn[i])); /// radi mi xor ln i f // a u f-u mi je radjen xor e i kn
            ...
            string Encripted_Text = this.DoPermutation(R16L16, DESData.ip_1); //uradimi inverzno od r16 l16
        ...
        {
            string E_Rn_1 = this.E_Selection(Rn_1); //E(Rn-1)

            string XOR_Rn_1_Kn = this.XOR(E_Rn_1, Kn);////xor-uj E(Rn-1) x Kn

            string sBoxedText = this.sBox_Transform(XOR_Rn_1_Kn); //s-uj mi B1 B2 ....s1(B1)s2(b2)...

            string P_sBoxedText = this.P(sBoxedText);// p od s-ova P(s1(B1)s2(b2)....)
